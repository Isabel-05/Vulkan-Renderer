...
namespace MaterialUtils
{
	...
	{
		auto vertShaderCode = BufferUtils::readFile(vertShaderPath);
		auto fragShaderCode = BufferUtils::readFile(fragShaderPath);
		...
		VkDescriptorSetLayout setLayouts[] = { cameraDSLayout };
		...
		if (vkCreatePipelineLayout(context.logicalDevice, &pipelineLayoutInfo, nullptr, &outPipelineLayout) != VK_SUCCESS) {
		...
		if (vkCreateGraphicsPipelines(context.logicalDevice, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &outPipeline) != VK_SUCCESS) {
		...
		VkPrimitiveTopology topology, VkPolygonMode polygonMode, VkPipeline& outPipeline, VkPipelineLayout& outPipelineLayout)
	{
		...
		pipelineRenderingInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO_KHR;
		pipelineRenderingInfo.colorAttachmentCount = 1;
		pipelineRenderingInfo.pColorAttachmentFormats = &idFormat;
		pipelineRenderingInfo.depthAttachmentFormat = VK_FORMAT_D32_SFLOAT;
