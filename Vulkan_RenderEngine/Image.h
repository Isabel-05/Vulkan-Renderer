...
namespace ImageUtils
{
	void createImage(VulkanContext& context, uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage,
		VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory, uint32_t mipLvls = 1, VkSampleCountFlagBits numsamples = VK_SAMPLE_COUNT_1_BIT);
	...
	void transitionImageLayout(VulkanContext& context, CommandPool& cmdPool, VkImage image,
		VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLvls);

	void transitionImageLayout(VulkanContext& context, VkCommandBuffer& cmdBuffer, VkImage image,
		VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLvls);
}
